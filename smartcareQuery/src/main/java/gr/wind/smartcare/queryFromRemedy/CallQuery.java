...
public class CallQuery {
    ...
    {
        CallQueryResponse response = new CallQueryResponse();
        ...
        try
        {
            ...
            List<Map<String, String>> csvData = carbonQueryExecution.run(
                    SmartcareQueryBuilder.QueryType.call_query, userName, password,
                    userTimestamp, userCli, requestID, requestTimestamp, remedyUser, systemId
            );

            // Convert response data into the response model
            List<CallQueryResponseModel> queryResponseData = CallQueryResponseModel.getCallResponseFromCsv(csvData, appConfig);
